...
class Welcome extends CI_Controller {
    ...
    function BACKPACK(){
        $config['base_url']         = site_url('Welcome/BACKPACK');
        ...
        $data['data']               = $this->Model_barang->get_barang_where($id);
        $this->load->view('template', $data);        
    }
    
    
    function TENDA(){
        $config['base_url']         = site_url('Welcome/TENDA');
        $config['total_rows']       = $this->Model_barang->paging()->num_rows();
        $config['per_page']         = 8;
        $config["uri_segment"]      = 3;
        $choice                     = $config["total_rows"] / $config["per_page"];
     ...
     function SLEEPINGBAG(){
        $config['base_url']         = site_url('Welcome/SLEEPINGBAG');
        $config['total_rows']       = $this->Model_barang->paging()->num_rows();
        $config['per_page']         = 8;
        $config["uri_segment"]      = 3;
        $choice                     = $config["total_rows"] / $config["per_page"];
